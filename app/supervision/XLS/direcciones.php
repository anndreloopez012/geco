...
$arrGestion = array();
$stmt = "SELECT C.CODICLIE, C.CLAPROD, C.NOMBRE, C.TIPOLOGI, C.CONCLUSI, C.RTESTADO, C.SUBCONCL, C.ESTADO, C.IDENTIFI, C.NIT, T.DIRECCION, T.ACTIVO, T.ORIGEN, T.PROPIETARIO, C.GESTORD, T.NIU 
        FROM $buscarBaseC C
        LEFT JOIN DIRECCIONES T
...
while ($rTMP = ibase_fetch_assoc($v_query)) {
    ...
    $arrGestion[$rTMP["CODICLIE"]]["IDENTIFI"]             = $rTMP["IDENTIFI"];
    $arrGestion[$rTMP["CODICLIE"]]["NIT"]             = $rTMP["NIT"];
    $arrGestion[$rTMP["CODICLIE"]]["DIRECCION"]             = $rTMP["DIRECCION"];
    $arrGestion[$rTMP["CODICLIE"]]["ACTIVO"]             = $rTMP["ACTIVO"];
    $arrGestion[$rTMP["CODICLIE"]]["ORIGEN"]             = $rTMP["ORIGEN"];
    $arrGestion[$rTMP["CODICLIE"]]["PROPIETARIO"]             = $rTMP["PROPIETARIO"];
    $arrGestion[$rTMP["CODICLIE"]]["GESTORD"]             = $rTMP["GESTORD"];
    $arrGestion[$rTMP["CODICLIE"]]["NIU"]             = $rTMP["NIU"];
...
<table cellspacing="0" cellpadding="0">
    <thead>
        <tr style="background:#D6EAF8; color:black;">
            <td width=%20>CODIGO CLIENTE</td>
            <td width=%20>CLAVE DE PRODUCTO</td>
    ...
    <tbody>
        ...
        ?>
                <tr>
                    ...
                    <td class='text'><?php echo  $rTMP["value"]['NIT']; ?></td>
                    <td><?php echo  $rTMP["value"]['DIRECCION']; ?></td>
                    <td class='text'><?php echo  $rTMP["value"]['ACTIVO']; ?></td>
                    <td ><?php echo  $rTMP["value"]['ORIGEN']; ?></td>
                    <td ><?php echo  $rTMP["value"]['PROPIETARIO']; ?></td>
                    <td class='text'><?php echo  $rTMP["value"]['GESTORD']; ?></td>
