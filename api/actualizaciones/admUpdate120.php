...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "insert") {
        ...
        foreach ($contactList as $key => $contactData) {
            ...
            $rsNIUU = ibase_query("SELECT NUMTRANS FROM GC000001 WHERE CODICLIE = '$contactData[0]' AND NUMEMPRE = $empresa");
            if ($row = ibase_fetch_row($rsNIUU)) {
                $idRow0 = trim($row[0]);
            }
            $NUMTRANS = isset($idRow0) ? $idRow0 : 0;

            $rsNIU = ibase_query("SELECT COUNT(NUMTRANS) FROM GC000001 WHERE CODICLIE = '$contactData[0]' AND NUMEMPRE = $empresa");
            ...
            if ($bollExiste >= 1) {
                $var_consulta = "UPDATE GC000001 SET CICLOACQ = $contactData[1], ESTADO = $contactData[3] WHERE NUMTRANS = '$NUMTRANS'";
