...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "insert") {
        ...
        foreach ($contactList as $key => $contactData) {
            ...
            $rsNIU = ibase_query("SELECT COUNT(NIU) FROM EMAILS WHERE CODICLIE = '{$contactData[0]}' AND EMAIL = '{$contactData[1]}'");
            if ($row = ibase_fetch_row($rsNIU)) {
                $idRow = trim($row[0]);
            }
            ...
            if ($numeroExiste == 0) {
            $var_consulta = "INSERT INTO EMAILS (NIU, CODICLIE, EMAIL) VALUES (0, '$contactData[0]', '{$contactData[1]}');";
            $query = ibase_prepare($var_consulta);
            $val = 1;
            if ($v_query = ibase_execute($query)) {
                echo $val;
